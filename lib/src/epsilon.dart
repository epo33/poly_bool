...
class Epsilon {
  ...
  Intersection? linesIntersect(
    Coordinate a0,
    Coordinate a1,
    Coordinate b0,
    Coordinate b1,
  ) {
    IntersectionPos along(
      Coordinate pt,
      Coordinate start,
      Coordinate end,
      double parametrizedCoord,
    ) {
      if (pointsSame(pt, start)) return IntersectionPos.atStart;
      if (pointsSame(pt, end)) return IntersectionPos.atEnd;
      if (parametrizedCoord < 0) return IntersectionPos.beforeStart;
      if (parametrizedCoord > 1) return IntersectionPos.afterEnd;
      return IntersectionPos.inSegment;
    }
    ...
    return Intersection(
      pt: pt,
      alongA: along(pt, a0, a1, A),
      alongB: along(pt, b0, b1, B),
    );
  }
}
